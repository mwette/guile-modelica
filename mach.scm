...
(define modelica-spec
  (lalr-spec
   ...
   (grammar
    ...
    (short-class-specifier
     (ident "=" base-prefix type-specifier
	    opt-array-subscripts
	    opt-class-modification
	    comment
	    ($$ (sx-tail (sx-list '- $1 $3 $4 $5 $6))))
     (ident "=" "enumeration" "(" enum-list ")" comment
	    ($$ (sx-tail (sx-list '- $1 `(enum ,$5) $6))))
     (ident "=" "enumeration" "(" ":" ")" comment
	    ($$ (sx-tail (sx-list '- $1 `(enum) $6))))
     )
    ...
    (opt-array-subscripts ($empty) (array-subscripts))
    (opt-class-modification ($empty) (class-modification))
    ...
    (component-declaration
     (declaration condition-attribute comment ($$ (append $1 (list $2 $3))))
     (declaration comment ($$ (if (null? $2) $1 (append $1 (list $2))))))
...
(define modelica-mach (make-lalr-machine modelica-spec))
