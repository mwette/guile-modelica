...
(define mo-act-v
  (vector
   ...
   ;; short-class-specifier => ident "=" base-prefix type-specifier opt-arr...
   (lambda ($7 $6 $5 $4 $3 $2 $1 . $rest)
   ...
   (lambda $rest (list))
   ;; opt-array-subscripts => array-subscripts
   (lambda ($1 . $rest) $1)
   ;; opt-class-modification => 
   (lambda $rest (list))
   ;; opt-class-modification => class-modification
   (lambda ($1 . $rest) $1)
